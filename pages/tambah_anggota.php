...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO anggota (nama_lengkap, alamat, no_telepon, email, password, foto_profil)
            VALUES (?, ?, ?, ?, ?, ?)";

    if ($stmt = $mysqli->prepare($sql)) {
        $stmt->bind_param("ssssss", $nama_lengkap, $alamat, $no_telepon, $email, $password, $foto_profil);

        if ($stmt->execute()) {
            $pesan = "Data anggota dengan nama <b> $nama_lengkap </b> berhasil ditambahkan.";
        } else {
            $pesan_error = "Gagal menambahkan data anggota: " . $mysqli->error;
        }
        $stmt->close();
    } else {
        $pesan_error = "Gagal menyiapkan query: " . $mysqli->error;
    }
}
?>
